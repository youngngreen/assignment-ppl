...
class StaticChecker(BaseVisitor, Utils):
    ...
    def visitMethodDecl(self, ast: MethodDecl, o):
        ...
        for inst in insts:
            ...
            if type(inst) is ConstDecl:
                self.visit(inst, (Constant(), env))
            if type(inst) is Stmt:
                if type(inst) is not Return:
                    print("hello")
                    self.visit(inst, (False, env))
                else:
                    typeMethod = self.visit(IntType(), o) # please check return type!!!
                    self.visit(inst, (typeMethod, env))
    ...
    def visitBlock(self, ast: Block, o):
        ...
        insts = ast.inst
        [self.visit(inst, (inLoop, env)) for inst in insts]
